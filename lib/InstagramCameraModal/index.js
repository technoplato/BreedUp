...
export default class CameraModal extends React.Component {
        ...
        return (
          ...
          >
            <Camera
              ...
              onNewPhotoUri={this.onImageChosen}
            />
          </View>
        )
      case this.ModalState.AFTER_IMAGE:
  ...
  onImageChosen = path => {
    console.log('L198 path ===', path)
    ImageCropper.openCropper({
      path,
      width: IMAGE_SIZE,
      height: IMAGE_SIZE
    }).then(({ path }) => {
      console.log('L203 path ===', path)
      this.showApprovePicture(path)
    })
  }
  ...
  pickImageFromLibrary() {
    ImagePicker.launchImageLibrary(null, response => {
      if (response.didCancel) {
        ...
        this.onImageChosen(response.uri)
      }
    })
  }
}
